...
@Service
public class GoldenPitchDeckService {

    private static final Logger log = LoggerFactory.getLogger(GoldenPitchDeckService.class);
    ...
    public GoldenPitchDeckService(ReferenceVideoRepository repository,
    ...
    public UploadUrlResponse getUploadUrl(UserPrincipal principal) {
        requireAdmin(principal);
        Long tenantId = requireTenant();
        findOrCreateGoldenPitch(tenantId);
        S3PresignedService.UploadResult result = s3PresignedService.generateUploadUrlGoldenPitchDeck(tenantId);
    ...
    @Transactional
    public void saveVideo(String key, UserPrincipal principal) {
        requireAdmin(principal);
        Long tenantId = requireTenant();
        ReferenceVideoEntity entity = findOrCreateGoldenPitch(tenantId);
    ...
    public GoldenPitchDeckResponse get(UserPrincipal principal) {
        requireAdmin(principal);
        requireTenant();
        return repository.findFirstByTypeOrderByIdAsc(ReferenceVideoType.GOLDEN_PITCH)
                .filter(e -> e.getVideoS3Key() != null && !e.getVideoS3Key().isBlank())
                .map(e -> GoldenPitchDeckResponse.builder()
                        .videoPlaybackUrl(s3PresignedService.generatePlaybackUrl(e.getVideoS3Key()))
                        .build())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Golden pitch deck video not set"));
    }
    ...
    public void delete(UserPrincipal principal) {
        ...
        ReferenceVideoEntity entity = repository.findFirstByTypeOrderByIdAsc(ReferenceVideoType.GOLDEN_PITCH)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Golden pitch deck not found"));
        ...
        // Delete videos from S3 (reference + all sales submissions)
        s3PresignedService.deleteObject(s3Key);
        submissionS3Keys.forEach(s3PresignedService::deleteObject);

        log.info("Deleted golden pitch deck (ref {}) with {} submissions", refId, submissions.size());

        // Recompute metrics for all affected sales users
    ...
    private void recomputeMetricsForOpportunities(Set<Long> opportunityIds) {
        Set<Long> recomputedUsers = new java.util.HashSet<>();
    ...
    private ReferenceVideoEntity findOrCreateGoldenPitch(Long tenantId) {
        return repository.findFirstByTypeOrderByIdAsc(ReferenceVideoType.GOLDEN_PITCH)
                .orElseGet(() -> {
                    ReferenceVideoEntity newEntity = ReferenceVideoEntity.builder()
                            .tenantId(tenantId)
                            .type(ReferenceVideoType.GOLDEN_PITCH)
                            .name("Golden Pitch Deck")
                            .processed(false)
    ...
    private void requireAdmin(UserPrincipal principal) {
        ...
        if (!Role.ADMIN.name().equalsIgnoreCase(principal.getRole())) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Only admin can manage golden pitch deck");
        }
    }
